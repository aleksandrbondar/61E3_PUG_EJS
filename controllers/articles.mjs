...
const getArticlesHandler = (req, res) => {
  ...
  res.render('./../views-ejs/articles.ejs', { articles: articles.slice(0, 10), pages: Math.ceil(articles.length / 10), currectPage: 1 })
}

const getArticlesByPageHandler = (req, res) => {
  const { page } = req.params
  res.render('./../views-ejs/articles.ejs', { articles: articles.slice((page - 1) * 10, page * 10), pages: Math.ceil(articles.length / 10), currectPage: +page })
}
...
const getArticleByIdHandler = (req, res) => {
  ...
  res.render('./../views-ejs/articleById.ejs', { article: articles[articleId - 1] })
}
